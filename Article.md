...
Очередной графоманский опус на тему программирования на языке Haskell.

План
- Постановка задачи – казалось бы чего сложного – прочитать из переменных окружения несколько значений и составить из них структуру данных для удобства доступа.
- начнем тестировать очень просто и "в лоб" - напишем тестовый интерпретатор, способный работать не с переменными окружения, а с набором пар ключ-значение вместо Environment-а
- как говорит один мой знакомый, "мало вариативности". Н что-ж, добавим вариативности. Есть такой прием в тестровании - обратимость. Построение конфига из окружения - есть прямое преобразование типа Окружение -> Конфиг. Если бы у нас было обратное преобразование, то мы бы могли проверить инверсность. Если сначала конфиг преобразовать в набор пар ключ-знчение, а потом из них попытпться прочитать конфиг, то в итоге должны ведь получить исходный конфиг.
- но это только success случай мы протестировали, пока не ясно как будет себя вести функция чтения конфига, если в переменных окружения будет что-то отсутствовать. Например одно из значений. Но погодите-ка – ведь у нас же есть способ получить окружение в виде списка строк. Достаточно только удалить из нее одну (случайную) строку и попытаться прочитать конфиг
- от каких ошибок защищают такие defensive тесты? например если случайно переставить местами строки при посмтроении конфига - тесты это отловят. Либо если попытаться захардкодить какое-нибудь одно значение на этапе построения конфига. Изменение названия строк, из которых читаем конфиг такой тест тоже "отловит", но отловит тут в кавычках, потому что такое падение теста же не говорит о некорректности или неработоспособности программы, оно говорит лишь о том, что тесты нужно обновить, по сути прописав в процедуру генерации мокового оружения новые названия трок. Такова цена. Добавление нового элемента в конфиг, понятное дело, отловит компилятор в виде несовподения типов.
- Какие выводы: использование таких приемов как: инверсия, идемподентность, инвариантность - отличные инструменты для написания тестов, при условии, что генерацию случайных данных на входе берет на себя библиотека. Вариативность, как говорит мой знакомый - на высоте ;)